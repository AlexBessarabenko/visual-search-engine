...
| P@3 | 0.83 | 0.87 | -0.03 |
| P@5 | 0.72 | 0.82 | -0.10 |
| R@5 | 0.136 | 0.173 | -0.037 |
| P@10 | 0.52 | 0.58 | -0.06 |
| R@10 | 0.158 | 0.197 | -0.039 |
| mAP | 0.154 | 0.194 | -0.040 |
| Среднее время поиска (Flat), мс | 0.11 | 0.14 | -0.03 |

> *Оценка на 500 изображениях и 10 запросах, подобранных под реальные подписи датасета. «Золотая» разметка — substring matching в подписях. В поиске используется гибридное ранжирование: векторное сходство + пересечение ключевых слов.*
...
1. **Jina v5-omni-nano** (1.04B параметров, truncate_dim=256):
   ...
   - truncate_dim=256 снижает размерность с 768 до 256 и ускоряет поиск
   - На нашем наборе из 500 stock-изображений чисто векторный поиск давал скромные результаты; гибридное ранжирование подняло P@1 до 0.90

2. **CLIP ViT-B/32** (150M параметров):
   - Преимущество: быстрое построение эмбеддингов, зрелая экосистема и более точная семантика на stock-изображениях Conceptual Captions
   - Ограничение: фиксированное разрешение 224×224
   - С гибридным ранжированием CLIP немного обгоняет Jina по mAP (0.194 vs 0.154)

3. **FAISS Flat vs HNSW**:
   - Flat: точный поиск, O(N) по памяти, достаточно быстрый для N=500 (поиск <0.2 мс)
...
4. **Гибридный поиск**
   - Чисто векторный поиск плохо справляется с шумными stock-подписями и редкими запросами
   - Решение: отбираем top-50 векторных кандидатов и переранжируем по keyword overlap (вес ключевых слов 0.7)
   - Это резко повышает P@1 и mAP, сохраняя почти мгновенное время ответа
